...
```
gemini extensions install <repo-or-local-path-to-this-extension> --ref v0.6.1
```
...
form; check `gemini extensions install --help` on your install. `--ref v0.6.1` pins
...
```json
{
  "mcpServers": {
    "engineering-audit": {
      ...
      "args": [
        "--from",
        "git+https://github.com/rodlunt/engineering-audit@v0.6.1",
...
`@v0.6.1` pins the install to the current tagged release rather than the moving `main` branch;
